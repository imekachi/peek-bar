...
- When disabled, no Secondary Separator appears; only the Toggle Icon is present.
- When enabled, PeekBar shows the Secondary Separator `ǀ` in the menu bar.
...
- Default layout, left → right: `[always-hidden items]  ǀ  [normal-collapse items]  ›`. Positions are user-adjustable via ⌘-drag.
- The positioning rule holds regardless of drag position: everything to the left of `ǀ` is treated as always-hidden except the Toggle Icon — even if `ǀ` is dragged to the right of `›`, or becomes the leftmost item.
