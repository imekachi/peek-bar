...
- Glyphs are rendered by PeekBar, not vendored from the reference project: the Toggle Icon uses SF Symbols (`chevron.right`/`chevron.left`); the Primary Separator is a solid vertical line (the Secondary Separator, a semi-translucent dashed line, is defined in 0003). The application icon is designed fresh (see 0008).
