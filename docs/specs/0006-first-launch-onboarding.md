...
- The Settings window mechanics themselves (see 0002); this spec defines the onboarding content shown within it.

## Behavior / Requirements
- Onboarding content lives inside the Settings window (see 0002).
...
- On first launch, the Settings window opens automatically (see 0002) so the onboarding content is seen.
- The onboarding is informational, remains available in Settings afterward, and does not block use of the app.

## Domain terms
See docs/specs/CONTEXT.md. Uses: Toggle Icon, Secondary Separator, Normal-collapse zone, Always-hidden zone, ⌘-drag arrange.

## Decisions
- Onboarding is embedded in Settings (not a separate modal) and reused as always-available help, avoiding a throwaway first-run-only screen.
